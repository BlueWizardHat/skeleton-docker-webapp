...
public class CurrentUserRestController extends BaseRestController {
	...
	private static final ResponseEntity<UserDetails> NO_CONTENT = ResponseEntity.status(HttpStatus.NO_CONTENT).build();
	...
	public ResponseEntity<UserDetails> currentUser() {
		return SecurityContextHelper.getLoggedInUser()
				.map(u -> ResponseEntity.ok(new UserDetails(u.getUser(), roles())))
				.orElse(NO_CONTENT);
	}
